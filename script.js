...
// основной задачей этого класса является,создание объект товара 
class Good {
  // закидываем в конструктор значения полей объекта одного товара
  ...
  // объект например item который сразу разбирается с помощью деструкторизаци, то есть код выше можно записать следующим образом
  // constructor(item) {
  //   this.id = item.id
  ...
  rander() {
    ...
    //  оставил только div каталога  и корзину
...
class Goods {
  ...
  constructor(options) {
    ...
    this.GoodsForShow = []
    // определяем поле ссылок к api которые потребуются для работы класса
    ...
    // вызываем метод запускающий методы, которые должны сработать в момент создания класса
    this._init()
  }
    ...
    return (
      makeRequestFetch(this.url.goods)
        .then((data) => {
          ...
          //  В момент создания класса в поля this.allGoods и this.GoodsForShow мы заносим данные из data.
